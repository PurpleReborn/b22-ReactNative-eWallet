...
import {compare} from '../redux/actions/user';
import {useState} from 'react';
import {connect, useSelector, useDispatch} from 'react-redux';

const Change = ({props, navigation}) => {
  ...
  const onSubmit = () => {
    if (formData.password.length >= 8) {
      dispatch(compare(token, formData, navigation));
    } else {
  ...
  return (
    <View style={styles.parent}>
      <View style={styles.parent2}>
        ...
        <Text style={styles.h2}>Masukan Security Code Anda saat ini</Text>
...
const mapDispatchToProps = {compare};
